...
final class IRFactory
{
    ...
    Node createSwitch(Node expr, int lineno, int offset) // <jgillick>
    {
        //
        ...
        Node.Jump switchNode = new Node.Jump(Token.SWITCH, expr, lineno, offset);
    ...
    Node createVariables(int token, int lineno, int offset) // <jgillick>
    {
        return new Node(token, lineno, offset); // <jgillick>
    }

    Node createExprStatement(Node expr, int lineno, int offset) // <jgillick>
    {
        int type;
        ...
        return new Node(type, expr, lineno, offset); // <jgillick>
    }

    Node createExprStatementNoReturn(Node expr, int lineno, int offset) // <jgillick>
    {
        return new Node(Token.EXPR_VOID, expr, lineno, offset); // <jgillick>
    }

    Node createDefaultNamespace(Node expr, int lineno, int offset) // <jgillick>
    {
        // default xml namespace requires activation
        setRequiresActivation();
        Node n = createUnary(Token.DEFAULTNAMESPACE, expr);
        Node result = createExprStatement(n, lineno, offset); // <jgillick>
        return result;
    }
    ...
    Node createName(String name){
    	return createName(name, -1, -1); // <jgillick>
    }
    Node createName(String name, int lineno, int offset) // <jgillick>
    {
        checkActivationName(name, Token.NAME);
        return Node.newString(Token.NAME, name, lineno, offset);
    }
     ...
     * @param offset the index in the source of the catch clause
     */
    Node createCatch(String varName, Node catchCond, Node stmts, int lineno, int offset) // <jgillick>
    {
        if (catchCond == null) {
            catchCond = new Node(Token.EMPTY);
        }
        return new Node(Token.CATCH, createName(varName),
                        catchCond, stmts, lineno, offset); // <jgillick>;
    }

    /**
     * Throw
     */
    Node createThrow(Node expr, int lineno, int offset) // <jgillick>
    {
        return new Node(Token.THROW, expr, lineno, offset); // <jgillick>
    }

    /**
     * Return
     */
    Node createReturn(Node expr, int lineno, int offset) // <jgillick>
    {
        return expr == null
            ? new Node(Token.RETURN, lineno, offset) /* <jgillick> */
            : new Node(Token.RETURN, expr, lineno, offset); // <jgillick>
    }

    /**
     * Debugger
     */
    Node createDebugger(int lineno, int offset) // <jgillick>
    {
        return new Node(Token.DEBUGGER,  lineno, offset); // <jgillick>
    }

    /**
     * Label
     */
    Node createLabel(int lineno, int offset) // <jgillick>
    {
        return new Node.Jump(Token.LABEL, lineno, offset); // <jgillick>
    }
     ...
     */
    Node createBreak(Node breakStatement, int lineno, int offset) // <jgillick>
    {
        Node.Jump n = new Node.Jump(Token.BREAK, lineno, offset); // <jgillick>
     ...
     */
    Node createContinue(Node loop, int lineno, int offset) // <jgillick>
    {
        if (loop.getType() != Token.LOOP) Kit.codeBug();
        Node.Jump n = new Node.Jump(Token.CONTINUE, lineno, offset); // <jgillick>
     ...
     */
    Node createBlock(int lineno, int offset) // <jgillick>
    {
        return new Node(Token.BLOCK, lineno, offset); // <jgillick>
    }
     ...
     */
    Node createScopeNode(int token, int lineno, int offset) // <jgillick>
    {
        return new Node.Scope(token, lineno, offset); // <jgillick>
    }
     ...
     */
    Node createLoopNode(Node loopLabel, int lineno, int offset) // <jgillick>
    {
        Node.Jump result = new Node.Scope(Token.LOOP, lineno, offset); // <jgillick>
        if (loopLabel != null) {
                            ...
                            Node init, Node incr)
    {
        ...
        if (loopType == LOOP_WHILE || loopType == LOOP_FOR) {
            ...
            loop.addChildrenToBack(new Node(Token.EMPTY, loop.getLineno(), loop.getOffset())); // <jgillick>
        }
    ...
    Node createTryCatchFinally(Node tryBlock, Node catchBlocks,
                               Node finallyBlock, int lineno, int offset) // <jgillick>
    {
        ...
        Node.Jump pn = new Node.Jump(Token.TRY, tryBlock, lineno, offset); // <jgillick>
            ...
            while (cb != null) {
                ...
                if (cond.getType() == Token.EMPTY) {
                    ...
                    condStmt = createIf(cond, catchStatement, null,
                                        catchLineNo, cb.getOffset()); // <jgillick>
                }
     ...
     * With
     */
    Node createWith(Node obj, Node body, int lineno, int offset) // <jgillick>
    {
        setRequiresActivation();
        Node result = new Node(Token.BLOCK, lineno, offset);  // <jgillick>
        result.addChildToBack(new Node(Token.ENTERWITH, obj));
        Node bodyNode = new Node(Token.WITH, body, lineno, offset);  // <jgillick>
     ...
     * DOTQUERY
     */
    public Node createDotQuery (Node obj, Node body, int lineno, int offset) // <jgillick>
    {
        setRequiresActivation();
        Node result = new Node(Token.DOTQUERY, obj, body, lineno, offset);  // <jgillick>
        return result;
    }
     ...
     * If statement
     */
    Node createIf(Node cond, Node ifTrue, Node ifFalse, int lineno, int offset) // <jgillick>
    {
    ...
    Node createYield(Node child, int lineno, int offset) // <jgillick>
    {
      if (!parser.insideFunction()) {
      ...
      setIsGenerator();
      if (child != null)
        return new Node(Token.YIELD, child, lineno, offset); // <jgillick>
      else
        return new Node(Token.YIELD, lineno, offset); // <jgillick>
    }
    ...
    Node createPropertyGet(Node target, String namespace, String name, int memberTypeFlags){
    	return createPropertyGet(target, namespace, name, memberTypeFlags, -1, -1);
    }

    Node createPropertyGet(Node target, String namespace, String name,
                           int memberTypeFlags, int lineno, int offset) // <jgillick>
    {
        if (namespace == null && memberTypeFlags == 0) {
            if (target == null) {
                return createName(name, lineno, offset);
            }
            checkActivationName(name, Token.GETPROP);
            if (ScriptRuntime.isSpecialProperty(name)) {
                Node ref = new Node(Token.REF_SPECIAL, target);
                ref.putProp(Node.NAME_PROP, name);
                return new Node(Token.GET_REF, ref, lineno, offset);
            }
            return new Node(Token.GETPROP, target, createString(name), lineno, offset);
        }
        Node elem = createString(name);
        memberTypeFlags |= Node.PROPERTY_FLAG;
        return createMemberRefGet(target, namespace, elem, memberTypeFlags, lineno, offset);
    }
    ...
    private Node createMemberRefGet(Node target, String namespace, Node elem, int memberTypeFlags){
    	return createMemberRefGet(target, namespace, elem, memberTypeFlags, -1, -1);
    }
                                    ...
                                    int memberTypeFlags, int lineno, int offset) // <jgillick>
    {
        ...
        if (namespace != null) {
            ...
            if (namespace.equals("*")) {
                nsNode = new Node(Token.NULL, lineno, offset);
            } else {
                nsNode = createName(namespace, lineno, offset);
            }
        }
        Node ref;
        if (target == null) {
            if (namespace == null) {
                ref = new Node(Token.REF_NAME, elem, lineno, offset);
            } else {
                ref = new Node(Token.REF_NS_NAME, nsNode, elem, lineno, offset);
            }
        } else {
            if (namespace == null) {
                ref = new Node(Token.REF_MEMBER, target, elem, lineno, offset);
            } else {
                ref = new Node(Token.REF_NS_MEMBER, target, nsNode, elem, lineno, offset);
            }
        }
